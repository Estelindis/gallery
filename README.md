...
- Set environment variables.  If using GitPod, variables can be accessed by clicking your user portrait in the top right of the screen, choosing User Settings from the dropdown, then choosing Variables in the list that appears on the right of the page.  Local debugging can be enabled while preventing live deployment in a state of ```Debug=True```, by creating an environment variable called DEVELOPMENT and setting this to True in GitPod.  Note that, even if this variable is set True locally, it should not be set at all when deploying to Heroku.  By not setting the DEVELOPMENT variable on Heroku, the live site is prevented from showing debugging messages instead of the intended error pages.  To handle Stripe payments, set up three further environment variables: STRIPE_PUBLIC_KEY, STRIPE_SECRET_KEY, and STRIPE_WH_SECRET.  These values must be kept secret and should be determined by your personal Stripe settings.
...
- Note: when deploying to Heroku, it will be necessary to return to your IDE briefly; change local database settings to those outlined above (e.g. for Elephant SQL), commenting out previous settings; do not commit this change; migrate the database; then revert the local IDE database setting changes to their previous values.
